...
class Calendar extends Component {
    ...
    renderDay(day, id) {
        ...
        let month_;
        let day_;
        if (this.state.currentMonth.toString('MM')[0] === '0')
            month_ = day.toString('MM')[1];
        else
            month_ = day.toString('MM');
        if (day.toString('dd')[0] === '0')
            day_ = day.toString('dd')[1];
        else
            day_ = day.toString('dd');
        const fateData = fateDB.fate.filter(fate => fate.cd_sy == day.toString('yyyy') && fate.cd_sm === month_ && fate.cd_sd === day_)[0];
        return (
            <View style={this.style.dayContainer} key={id}>
                ...
                />
                {
                    !dateutils.sameMonth(day, this.state.currentMonth) ? <Text></Text> : (
                        <View style={[{ flexDirection: 'row' }, { alignItems: 'center' }]}>
                            {
                                day_ === "1" || fateData.cd_ld === "1" ?
                                    <Text style={[{ fontSize: 10 }, { flex: 1 }, { marginLeft: 2 }, {color: 'red'}, {fontWeight: '600'}]}>{fateData.cd_lm}.{fateData.cd_ld}</Text>
                                    :
                                    <Text style={[{ fontSize: 10 }, { flex: 1 }, { marginLeft: 2 }]}>{fateData.cd_ld}</Text>
                            }
                            <Text style={[]}>{fateData.cd_hdganjee}</Text>
